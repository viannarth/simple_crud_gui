...
Clone the repository in a desired directory with `git clone` and run in `python install -r requirements.txt` to install the project dependencies. It is recommended to create a Python virtual environment. Then, simply run `python app.py` to start the program.
